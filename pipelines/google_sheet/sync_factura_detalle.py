...
from extractors.odoo.invoices import get_invoice_lines_raw, get_invoices_raw
from transform.google_sheets.f_detalle import transform_facturas_detalle
...
def run_sync_factura_cabecera():
    ...
    SHEET_NAME = "Facturas Detalle"

    try:
        # 1. Conexión
        client = get_odoo_client()

        # Se calcula la fecha del mes 
        ...
        raw_cabeceras = get_invoices_raw(client, fecha_inicio=fecha_filtro_str)
        raw_data = get_invoice_lines_raw(client, fecha_inicio=fecha_filtro_str)
        ...
        if not df_filtered.empty:
            ...
            df_to_upload = transform_facturas_detalle(data_to_transform, cabeceras_raw=raw_cabeceras)
        else:
        ...
        if not df_to_upload.empty:
            ...
            upload_dataframe_to_sheet(df_to_upload, SPREADSHEET_ID, SHEET_NAME)
